...
void main()
{
  ...
  auto dp = new mint[][](n, k+1), dps = new mint[][](n, k+2);
  dp[0][0] = 1;
  foreach (i; 1..k+2) dps[0][i] = 1;

  foreach (i; 1..n) {
    foreach (j; 0..min(k, i*(i+1)/2)+1)
      dp[i][j] = dps[i-1][j+1] - dps[i-1][max(0, j-i)];

    foreach (j; 1..k+2) dps[i][j] = dps[i][j-1] + dp[i][j-1];
  }
