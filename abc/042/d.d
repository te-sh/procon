...
void main()
{
  auto rd = readln.split.to!(int[]), h = rd[0], w = rd[1], a = rd[2], b = rd[3];

  auto hw = h+w;

  auto fact = new mint[](hw);
  fact[0] = 1;
  foreach (i; 1..hw) fact[i] = fact[i-1] * i;

  auto invFact = new mint[](hw);
  invFact[$-1] = fact[$-1].inv;
  foreach_reverse (i; 1..hw) invFact[i-1] = invFact[i] * i;

  auto ans = fact[h-1+w-1] * invFact[h-1] * invFact[w-1];
  foreach (i; 0..b)
    ans -= fact[h-a-1+i] * invFact[h-a-1] * invFact[i] * fact[a-1+w-1-i] * invFact[a-1] * invFact[w-1-i];
