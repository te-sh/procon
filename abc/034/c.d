...
void main()
{
  auto rd = readln.split.to!(int[]), w = rd[0]-1, h = rd[1]-1;

  auto fact = new mint[](w+h+1);
  fact[0] = 1;
  foreach (i; 1..w+h+1) fact[i] = fact[i-1] * i;

  auto invFact = new mint[](w+h+1);
  invFact[w+h] = fact[w+h].inv;
  foreach_reverse (i; 1..w+h) invFact[i] = invFact[i+1] * (i+1);

  writeln(fact[w+h] * invFact[w] * invFact[h]);
}
