...
void main()
{
  ...
  auto d = new int[](n);
  foreach (i; 0..n-1) d[i] = readln.chomp.to!int;

  auto f = new int[](1<<k);
  foreach (i; 0..1<<k)
    foreach (j; 0..k-1)
      if (i.bitTest(j) ^ i.bitTest(j+1)) ++f[i];

  auto dp = new mint[][][](n+1, 1<<k, m);
  dp[0][0][0] = 1;

  foreach (i; 0..n)
    foreach (j; 0..1<<k)
      foreach (u; 0..m) {
        dp[i+1][j][(u+d[i]*f[j]) % m] += dp[i][j][u];
        foreach (v; 0..k) {
          if (j.bitTest(v)) continue;
          auto j2 = j.bitSet(v);
          auto u2 = (u + d[i]*f[j2]) % m;
          dp[i+1][j2][u2] += dp[i][j][u];
        }
      }

  writeln(dp[n][(1<<k)-1][0]);
}
