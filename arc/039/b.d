...
void main()
{
  ...
  auto fact = new mint[](n+k);
  fact[0] = 1;
  foreach (i; 1..n+k) fact[i] = fact[i-1] * i;
  auto invFact = new mint[](n+k);
  invFact[n+k-1] = fact[n+k-1].inv;
  foreach_reverse (i; 1..n+k) invFact[i-1] = invFact[i] * i;

  if (n > k)
    writeln(fact[n+k-1] * invFact[k] * invFact[n-1]);
  else
    writeln(fact[n] * invFact[k%n] * invFact[n-k%n]);
}
